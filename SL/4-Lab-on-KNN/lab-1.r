...
KNN_multi = function(x0, x, y, K, C) {
    distances = YOUR_ANSWER_HERE  # Euclidean distance between x0 and each x[,i]
    o = order(distances)  # order of the training points by distance from x0 (nearest to farthest)
    p_hat = sapply(1:C, function(j){ YOUR_ANSWER_HERE })  # p_hat[j] = proportion of y values of the K nearest training points that are equal to j.
...
KNN_multi_predict = function(x0, x, y, K, C) {
    ...
    y0 = YOUR_ANSWER_HERE  # find the class with the highest estimated probability
...
# Report your results:
# (a) d=2
#    training error rate = YOUR_ANSWER_HERE
...
#    naive_test_error = YOUR_ANSWER_HERE
#    optimal_test_error = YOUR_ANSWER_HERE
#
